...
export function EnglishReader({ items }: { items: EnglishItem[] }) {
  ...
  return (
    <div className="grid min-h-[calc(100vh-8rem)] gap-5 lg:grid-cols-[250px_minmax(0,1fr)]">
      <div className="hidden overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 lg:block"><EnglishSidebar items={items} selectedId={selected.id} onSelect={select} /></div>
      ...
      <main className="min-w-0 overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-soft dark:border-slate-800 dark:bg-slate-900">
        <div className="border-b border-slate-100 px-5 py-4 dark:border-slate-800 sm:px-8 sm:py-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h1 className="min-w-0 text-xl font-semibold tracking-tight text-slate-900 dark:text-white sm:text-2xl">{selected.title}</h1><button onClick={() => setMobileOpen(true)} aria-label="Open lessons" className="shrink-0 self-end rounded-lg border border-slate-200 p-2 text-slate-500 lg:hidden sm:self-auto"><ListBulletIcon /></button></div>
        </div>
